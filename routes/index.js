...
function loadService(){
    ...
    moduleNames.forEach(file => {
        ...
        serviceModules.set(moduleName, _module);
    });
    logger.info('service模块加载完成！');
}
...
function initModule(_module, app, file){
    ...
    if(Array.isArray(routes)){
        ...
        routes.forEach(route => {
            let { path, method = 'get', action, service } = route;
            if(path == null || action == null || service == null){
                return void logger.warn(`路由配置文件${file}：路由配置文件path、action、service不能为空`);
            }
            method = method.toLowerCase();
            if(!_methods.includes(method)){
                return void logger.warn(`路由配置文件${file}：${path} -> 请求方法methods只能为${_methods.join('，')}`);
            }
            const serviceModule = serviceModules.get(service);
            if(serviceModule == null){
                return void logger.warn(`路由配置文件${file}：${path} -> service模块${service}不存在`);
            }
            const actionFn = serviceModule[action];
